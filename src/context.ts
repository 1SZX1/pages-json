...
export class Context {
  ...
  public async updatePagesJSON(filepath?: string): Promise<boolean> {
    ...
    const raw = await this.generatePlatformPagesJsonRAW({ platforms, indent, eof });
    if (this.lastPagesJson === raw) {
      debug.info('pages.json 无改动，跳过更新。');
      return false;
    }

    await writeFileWithLock(this.staticJsonFilePath, raw);

    this.lastPagesJson = raw;
  ...
  private async generatePlatformPagesJsonRAW({ platforms, indent, eof }: StaticJsonFileInfo): Promise<string> {
    ...
    const { pages = [], subPackages = [], tabBar, ...rest } = pagesJson;

    for (const platform of restPlatforms) {
      const json = await this.generatePagesJson(platform);
      // 合并不同平台的 pages
      if (json.pages) {
        mergePlatformArray(firstPlatform, pages, platform, json.pages, v => v.path);
      }
      // 合并不同平台的 subPackages
      if (json.subPackages) {
        for (const sub of json.subPackages) {
          const exist = subPackages.find(s => s.root === sub.root);
          if (exist) {
            if (sub.pages) {
              exist.pages = exist.pages || [];
              mergePlatformArray(firstPlatform, exist.pages, platform, sub.pages, v => v.path);
            }
          } else {
    ...
    this.sortPagesJson(finalJson);

    let rawJson = cjStringify(finalJson, null, indent) + eof;

...
export class Context {
  ...
  /**
   * 生成并合并多个平台的 pages.json
   *
   * ! 多平台的 key 有重复，定制了 key 后缀，所以只能返回 raw
   */
  private async generatePlatformPagesJsonRAW({ platforms, indent, eof }: StaticJsonFileInfo): Promise<string> {
    const [firstPlatform = currentPlatform, ...restPlatforms] = [...platforms].sort();
    const pagesJson = await this.generatePagesJson(firstPlatform);

    for (const platform of restPlatforms) {
      const json = await this.generatePagesJson(platform);
      // 合并不同平台的 pages
      if (json.pages) {
        pagesJson.pages = pagesJson.pages || [];
        mergePlatformArray(firstPlatform, pagesJson.pages, platform, json.pages, v => v.path);
      }
      // 合并不同平台的 subPackages
      if (json.subPackages) {
        pagesJson.subPackages = pagesJson.subPackages || [];
        for (const sub of json.subPackages) {
          const idx = pagesJson.subPackages.findIndex(s => s.root === sub.root);
          if (idx > -1) {
            const { pages, ...rest } = sub;
            mergePlatformObject(firstPlatform, pagesJson.subPackages[idx], platform, rest);
            if (pages && pages.length > 0) {
              pagesJson.subPackages[idx].pages = pagesJson.subPackages[idx].pages || [];
              mergePlatformArray(firstPlatform, pagesJson.subPackages[idx].pages, platform, pages, v => v.path);
            }
          } else {
      ...
      if (json.tabBar) {
        ...
        const { list, ...rest } = json.tabBar;
        mergePlatformObject(firstPlatform, pagesJson.tabBar, platform, rest);
        if (list && list.length > 0) {
          pagesJson.tabBar.list = pagesJson.tabBar.list || [];
          mergePlatformArray(firstPlatform, pagesJson.tabBar.list, platform, list, v => v.pagePath);
        }
      }
    }
    ...
    const comment = `// GENERATED BY @uni-ku/pages-json, PLATFORM: ${[firstPlatform, ...restPlatforms].join(' || ')} \n`;
  ...
  public async writePagesJson(jsons: Record<BuiltInPlatform, PagesJSON.PagesJson>, indent = ' '.repeat(4), eof = '\n'): Promise<boolean> {
    ...
    let pagesJson = jsons[p1] || {};

    const { pages: _, subPackages: __, tabBar: ___, ...j1rest } = pagesJson;

    for (const p2 of p2s) {
      const { pages: j2pages, subPackages: j2subPackages, tabBar: j2tabBar, ...j2rest } = jsons[p2];

      // 合并不同平台的 pages
      if (j2pages) {
        pagesJson.pages = pagesJson.pages || [];
        mergePlatformArray(p1, pagesJson.pages, p2, j2pages, v => v.path);
      }
      // 合并不同平台的 subPackages
      if (j2subPackages) {
        pagesJson.subPackages = pagesJson.subPackages || [];
        for (const j2Sub of j2subPackages) {
          const idx = pagesJson.subPackages.findIndex(s => s.root === j2Sub.root);
          if (idx > -1) {
            const { pages: _, ...rest } = pagesJson.subPackages[idx];
            const { pages: j2sPages, ...j2sRest } = j2Sub;
            mergePlatformObject(p1, rest, p2, j2sRest);
            pagesJson.subPackages[idx] = {
              ...pagesJson.subPackages[idx],
              ...rest,
            };
            if (j2sPages && j2sPages.length > 0) {
              pagesJson.subPackages[idx].pages = pagesJson.subPackages[idx].pages || [];
              mergePlatformArray(p1, pagesJson.subPackages[idx].pages, p2, j2sPages, v => v.path);
            }
          } else {
      ...
      if (j2tabBar) {
        pagesJson.tabBar = pagesJson.tabBar || {};
        const { list: _, ...rest } = pagesJson.tabBar;
        const { list: j2tbList, ...j2tbRest } = j2tabBar;
        mergePlatformObject(p1, rest, p2, j2tbRest);
        pagesJson.tabBar = {
          ...pagesJson.tabBar,
          ...rest,
        };
        if (j2tbList && j2tbList.length > 0) {
          pagesJson.tabBar.list = pagesJson.tabBar.list || [];
          mergePlatformArray(p1, pagesJson.tabBar.list, p2, j2tbList, v => v.pagePath);
        }
      }

      // 合并除 pages、subPackages、tabBar 外的其他属性
      mergePlatformObject(p1, j1rest, p2, j2rest);
    }
